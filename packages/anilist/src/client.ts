...
export interface IAniListClient {
  ...
  searchManga(query: string, page?: number): Promise<SearchResult>;
...
export class AniListClient implements IAniListClient {
   ...
   */
  async searchManga(query: string, page = 1): Promise<SearchResult> {
  ...
  async getUserMangaList(userId: number, status?: MediaListStatus): Promise<MediaListEntry[]> {
    ...
    const response = await this.request<{
      MediaListCollection: { lists: { entries: unknown[] }[] };
    }>(queries.GET_USER_MANGA_LIST, { userId, status });

    // Flatten entries from all lists
    const allEntries = response.MediaListCollection.lists.flatMap((list) => list.entries);
